...
$(function(){
    /*object.onclick = kattint(){elso};*/
    object.addEventListener("click", elso);
    object.addEventListener("click", masodik);
});
$(function kattint(){
    for (let i = 0; i < versTomb.length; i++) {
        const element = versTomb[i];
        $(".elem1").append("<div><h3>"+element["versCim"] + "</h3><img src='" + element.kep+ " ' alt = 'szerző'>" + "<p>element.vers</p>"+"<p>+element.szerzo</p></div>");
